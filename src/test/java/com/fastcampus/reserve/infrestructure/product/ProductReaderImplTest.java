...
class ProductReaderImplTest {
    ...
    public void whenAreaCodeIsEmpty_shouldCallFindAll() {
        // Given
        ProductListOptionDto dto = new ProductListOptionDto(LocalDate.now(), LocalDate.now().plusDays(2), null, "", 1, 10);
    ...
    public void whenAreaCodeProvided_shouldUseAreaCodeFilter() {
        ...
        ProductListOptionDto dto = new ProductListOptionDto(LocalDate.now(), LocalDate.now().plusDays(2), null, areaCode, 1, 10);
        List<Product> expectedProducts = Collections.singletonList(Product.builder().id(2531417L)
                .name("전주 한옥숙박체험관[한국관광 품질인증/Korea Quality]")
                .category("한옥")
                .description("전주 한옥숙박체험관은 전주한옥마을 중심부인 은행로에 있어 한옥마을을 여행하기에 수월하다. 골목을 따라 안쪽으로 들어가야 만날 수 있기 때문에 은행로의 번잡함은 걱정하지 않아도 된다. 전 객실은 온돌로 구성되어 있고 복층공간이 있어 아이들에게 인기가 좋다. 또한 정수기를 설치하여 편의성을 높였다. 조식으로 토스트와 아메리카노, 과일과 차를 제공하고, 야외 마당에는 테이블이 있어 휴식을 취하기 좋다.")
                .zipCode("55042")
